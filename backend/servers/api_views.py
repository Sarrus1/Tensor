...
def AsyncSSHCommand(body):
    ...
    sshCommand = "cd /home/{} && ./{} {}".format(
        server.user, server.instance_name, command)

    serverControl = ServerControlModel(
        uuid=uuid, output="Sending command...").save()

    client = paramiko.SSHClient()
    client.load_system_host_keys()
    client.set_missing_host_key_policy(paramiko.WarningPolicy())
    client.connect(hostname, port=sshPort,
    ...
    while True:
        ...
        serverControl.save()
    client.close()
    server = ServerControlModel.objects.filter(
        uuid=uuid).order_by("-id").first()
    server.status = 1
    server.save()
...
class ServerControlView(views.APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        controlStatus = ServerControlModel.objects.filter(
            uuid=uuid).order_by("-id").first()
