...
class BansFilter(django_filters.FilterSet):
    ...
    bid = django_filters.NumberFilter(
        field_name="bid", lookup_expr="iexact", label="bid")
            ...
            try:
                with valve.rcon.RCON(address, password) as rcon:
                    ...
                    if (steamid in ids):
                        ...
                        rcon.execute("sm_kick #{} You have been banned from this server by an admin.".format(
                            steamid.replace(":", "_")))
