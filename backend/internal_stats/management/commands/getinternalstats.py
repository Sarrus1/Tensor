from internal_stats.models import *
from gamestatistics.models import CkWrcps, Rank_awp, Rank_retake
from sourcebans.models import SbBans
from django.db.models import Sum
from django.core.management.base import BaseCommand


class Command(BaseCommand):
    help = 'Get internal server stats'

    def handle(self, *args, **options):
        awpUsers = list(Rank_awp.objects.values_list('steam', flat=True))
        retakesUsers = list(
            Rank_retake.objects.values_list('steam', flat=True))
        surfUsers = list(CkWrcps.objects.values_list('steamid', flat=True))
        allUsers = awpUsers + retakesUsers + surfUsers
        setAllUsers = set(allUsers)
        total_users = len(setAllUsers)

        # Time query
        days_played = 0
        awp_time = Rank_awp.objects.aggregate(
            Sum("connected"))["connected__sum"]
        if awp_time:
            days_played += awp_time
            retake_time = Rank_retake.objects.aggregate(Sum("connected"))[
                "connected__sum"]
            if retake_time:
                days_played += retake_time
            days_played = days_played//86400

            # Bans query
            total_bans = SbBans.objects.all().count()

            NewInternalStats = InternalStats(
                total_users=total_users, days_played=days_played, total_bans=total_bans)
            NewInternalStats.save()
