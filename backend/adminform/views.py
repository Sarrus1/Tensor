...
def ModeratorApplicationView(request):
    ...
    application = ApplicationModel.objects.filter(
        SteamID=steamid).order_by('-Date')
    if application:
        if application[0].Status == 'Rejected' or application[0].Status == 'Pending':
            messages.success(
                request, 'You can only apply once.', extra_tags='danger')
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                application.Name = SteamUser.objects.get(
                    steamid=commid).personaname
                form.save()
                send_mail(
                ...
                webhook = DiscordWebhook(
                    url=getenv("DISCORD_WEBHOOK_MODERATION"))
                ...
                embed.add_embed_field(
                    name='Server', value=application.Server.upper(), inline=True)
                embed.add_embed_field(name='Player Name', value=str(
                    SteamUser.objects.get(steamid=commid).personaname), inline=True)
                user_stats = Rank_awp.objects.get(
                    steam__contains=str(steamid[8:]))
                score = user_stats.score
                embed.add_embed_field(
                    name='Points', value=str(score), inline=True)
                ...
                messages.success(
                    request, 'Your application has been submitted.', extra_tags='success')
                return HttpResponseRedirect("/")
        else:
            form = ApplicationForm()
        return render(request, "adminform/moderator_application.html", {'form': form})
...
def ModeratorApplicationValidationView(request, steamid, server_name):
    if not IsStaff(request.user.steamid2):
        messages.success(
            request, "You don't have permission to do that.", extra_tags='danger')
        return HttpResponseRedirect("/")
    application = ApplicationModel.objects.get(
        Server=server_name, SteamID=steamid)
...
def AcceptApplication(request, steamid, server_name):
    ...
    if application.Status == 'Pending':
        ...
        embed.add_embed_field(
            name='Server', value=application.Server.upper(), inline=True)
        embed.add_embed_field(name='Player Name',
                              value=application.Name, inline=True)
        user_stats = Rank_awp.objects.get(steam__contains=steamid[8:])
        score = user_stats.score
        embed.add_embed_field(
            name='DiscordID', value=application.Discord, inline=True)
        embed.set_thumbnail(url=str(request.user.avatarfull))
        webhook.add_embed(embed)
        response = webhook.execute()
        messages.success(
            request, 'Successfuly accepted new Moderator', extra_tags='success')
...
def RefuseApplication(request, steamid, server_name):
    ...
    if application.Status == 'Pending':
        ...
        application.save()
        messages.success(
            request, 'Successfuly rejected Application', extra_tags='warning')
    return HttpResponseRedirect("/")
