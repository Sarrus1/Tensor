...
def AddToSourcebans(steamid, email, Type, server):
	...
	if server == 'awp':
		...
		server_group = 4
	if not (myresult):
		mycursor.execute("INSERT INTO `sb_admins` (`aid`, `user`, `authid`, `password`, `gid`, `email`, `validate`, `extraflags`, `immunity`, `srv_group`, `srv_flags`, `srv_password`, `lastvisit`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (aid, user, authid, password, gid, email, validate, extraflags, immunity, Type, srv_flags, srv_password, last_visit))
	else:
		aid = myresult[0][0]

	mycursor.execute("SELECT admin_id FROM `sb_admins_servers_groups` WHERE admin_id=%s AND group_id=%s AND srv_group_id=%s", (aid, group_id, server_group))
	myresult = mycursor.fetchall()

	if not (myresult):
		mycursor.execute("INSERT INTO `sb_admins_servers_groups` (`admin_id`, `group_id`, `srv_group_id`, `server_id`) VALUES (%s, %s, %s, '-1')", (aid, group_id, server_group))
